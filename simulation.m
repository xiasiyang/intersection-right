...
%    PacketLength(i) = 164;%初始化分组长度
    CW_1(i) = W1_0; %竞争窗口，CW = 32 slots
    BackoffTimer_1(i) = W1_0-1; %退避时间用 1200 slots来代表无穷大
end
...
%                  liancheng(j,k)=liancheng(j-1,k);
     ...
     PDR0_sim2(1,k)=PDR0s((P*Mv),k)/PDR0x((P*Mv),k);
     ...
     PDR1_sim2(1,k)=PDR1s((P*Mv),k)/PDR1x((P*Mv),k);
     ...
     PDR2_sim2(1,k)=PDR2s((P*Mv),k)/PDR2x((P*Mv),k);
     ...
     PDR3_sim2(1,k)=PDR3s((P*Mv),k)/PDR3x((P*Mv),k);
...
save('PDR0_sim2.mat','PDR0_sim2'); 
save('PDR1_sim2.mat','PDR1_sim2'); 
save('PDR2_sim2.mat','PDR2_sim2'); 
save('PDR3_sim2.mat','PDR3_sim2'); 
...
hold on;
x1=2:1:N;  
plot(0.1*x1,PDR0_sim2,':*b');
hold on;
x2=2:1:N;
plot(0.1*x2,PDR1_sim2,':+g');
hold on;
x3=2:1:N;  
plot(0.1*x3,PDR2_sim2,':or');
hold on;
x4=2:1:N;
plot(0.1*x4,PDR3_sim2,':xy');
title('PDR')
hold on;
